--=======================================================================
-- File Name    : boss_robot.lua
-- Creator      : yestein(yestein86@gmail.com)
-- Date         : 2014/9/2 20:31:45
-- Description  : ai robot
-- Modify       : 
--=======================================================================
if not BossRobot then
	BossRobot = ModuleMgr:NewModule("boss_robot")
end

BossRobot:DeclareListenEvent("GAME.AI_ACTIVE", "OnAIActive")
BossRobot:DeclareListenEvent("GAME.ACTION_OVER", "OnActionEnd")
BossRobot:DeclareListenEvent("GAME.ACTION_START", "OnActionStart")

function BossRobot:_Uninit()
	ModuleMgr:UnregisterUpdate(self:GetClassName())
	return 1
end

function BossRobot:_Init()
	return 1
end

function BossRobot:CanWork()
	if GameStateMachine:IsInEnemyAction() == 1 then
		return 1
	end
end

function BossRobot:OnActionStart()
	if self:CanWork() == 1 then
		ModuleMgr:RegisterUpdate(self:GetClassName(), "OnActive")
	end
end

function BossRobot:OnActionEnd()
	if self:CanWork() == 1 then
		ModuleMgr:UnregisterUpdate(self:GetClassName())
	end
end

function BossRobot:OnActive(frame)
	if frame % 1 == 0 then
...
function BossRobot:ThinkAndOperate()
